...
// Hostname of a thumbnail URL ("" when it isn't a parseable absolute URL).
function thumbHost(url) {
  try { return new URL(url).hostname.toLowerCase(); } catch { return ""; }
}

// Google's artwork CDNs. These are host checks on purpose — a substring test such as
// url.includes("googleusercontent.com") also matches https://evil.com/googleusercontent.com/x.
const isGoogleArtHost = (host) =>
  host === "googleusercontent.com" || host.endsWith(".googleusercontent.com") ||
  host === "ggpht.com" || host.endsWith(".ggpht.com");
const isYtimgHost = (host) => host === "ytimg.com" || host.endsWith(".ytimg.com");

export const isGoogleArtUrl = (url) => isGoogleArtHost(thumbHost(url));
...
export function hiResThumb(url, size = 512) {
  if (!url) return url;
  const host = thumbHost(url);
  if (isGoogleArtHost(host)) {
